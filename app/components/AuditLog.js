...
function MitreMatrix({ logs, onMitreFilter, onClose }) {
  ...
  function handleTechClick(tech, isSel) {
    setSelected(isSel ? null : tech.id)
  }

  function handleFilterClick(e, techId) {
    e.stopPropagation()
    onMitreFilter?.(techId)
    onClose?.()
  }
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'14px 20px' }}>
            ...
            <div key={tactic.id} style={{ display:'flex', flexDirection:'column', gap:'3px' }}>
              ...
              {byTactic[tactic.id]?.sort((a,b) => b.count - a.count).map(tech => {
                ...
                return (
                  <div
                    key={tech.id}
                    onClick={() => handleTechClick(tech, isSel)}
                    ...
                    title={`${tech.id} · ${tech.name} · ${tech.count} detection${tech.count !== 1 ? 's' : ''} — click to expand, double-click to filter history`}
                  >
                    ...
                    {isSel && (
                      <div style={{ marginTop:'4px', paddingTop:'4px', borderTop:`0.5px solid ${c.border}` }}>
                        ...
                        <div
                          onClick={(e) => handleFilterClick(e, tech.id)}
                          style={{
                            marginTop: '5px',
        ...
        <div style={{ marginTop:'10px', fontFamily:'var(--font-mono),monospace', fontSize:'8px', color:'var(--text-muted)', textAlign:'right' }}>
          CLICK TECHNIQUE TO EXPAND · CLICK FILTER HISTORY TO SEARCH
        </div>
      </div>
    </div>
  )
...
export default function AuditLog({ onClose, onMitreFilter }) {
      ...
      <div className="arb-modal" onClick={e => e.stopPropagation()}>
        ...
        {activeTab === 'heatmap' && (
          <div style={{ flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
            <MitreMatrix logs={logs} onMitreFilter={onMitreFilter} onClose={onClose} />
          </div>
        )}
          ...
          <div className="arb-modal-body">
            ...
            <div className="arb-modal-detail">
              <div style={{ padding:'16px 20px', display:'flex', flexDirection:'column', gap:'14px' }}>
                ...
                {entry.triage.indicators?.length > 0 && (
                  <div>
                    <div style={{ fontFamily:'var(--font-mono),monospace', fontSize:'8px', color:'var(--text-muted)', letterSpacing:'0.15em', textTransform:'uppercase', marginBottom:'8px' }}>TECHNICAL INDICATORS</div>
                    <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
                      {entry.triage.indicators.map((ind, i) => (
                        <div key={i} style={{ display:'flex', gap:'8px', alignItems:'flex-start' }}>
                          <span style={{ fontFamily:'var(--font-mono),monospace', fontSize:'8px', color:'var(--amber)', flexShrink:0, marginTop:'2px' }}>▸</span>
                          <span style={{ fontFamily:'var(--font-mono),monospace', fontSize:'10px', color:'var(--text-secondary)', lineHeight:'1.5' }}>{ind}</span>
                        </div>
                      ))}
                    </div>
                  </div>
